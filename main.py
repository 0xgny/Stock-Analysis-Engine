...
def compare_stocks(symbols: list, period: str = "2y"):
    ...
    try:
        ...
        if len(all_stats) > 1:
            ...
            comp_plot = visualizer.plot_comparative_stats(
                list(all_stats.keys()), period
            )
            if comp_plot:
            ...
            if len(all_stats) >= 2:
                ml_results = ml_analyzer.comprehensive_analysis(
                    list(all_stats.keys()), period
                )
                ...
                if "visualizations" in ml_results:
                    ...
                    if viz["price_correlation_heatmap"]:
                        print(
                            f" Price correlation heatmap: {viz['price_correlation_heatmap']}"
                        )
                    if viz["returns_correlation_heatmap"]:
                        print(
                            f" Returns correlation heatmap: {viz['returns_correlation_heatmap']}"
                        )
                    if viz["regression_analysis_plot"]:
                        print(
                            f" Regression analysis plots: {viz['regression_analysis_plot']}"
                        )
                    if viz["rolling_correlations_plot"]:
                        print(
                            f" Rolling correlations plot: {viz['rolling_correlations_plot']}"
                        )
            ...
            print(f"{'─' * 70}")
            print(
                f"{'Stock':<8} {'Vol%':<8} {'Skew':<8} {'Kurt':<8} {'Return%':<10} {'Sharpe':<8}"
            )
            print(f"{'─' * 70}")

            for symbol, stats in all_stats.items():
                print(
                    f"{symbol:<8} {stats['historical_volatility']:<8.2f} "
...
def main():
    ...
    try:
        ...
        if choice == "1":
            ...
            symbols_input = input(
                f"Enter symbols separated by commas [default: {', '.join(default_stocks)}]: "
            ).strip()
            if symbols_input:
