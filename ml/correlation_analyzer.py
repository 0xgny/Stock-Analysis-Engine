...
class CorrelationAnalyzer:
    ...
    def __init__(self):
        ...
        self.output_dir = os.path.join(
            os.path.dirname(__file__), "..", "output", "ml_plots"
        )
        ...
        if len(symbols) >= 2:
            for i, symbol1 in enumerate(symbols):
                for symbol2 in symbols[i + 1 :]:
                    if (
                        symbol1 in returns_data.columns
                        and symbol2 in returns_data.columns
                    ):
                        rolling_corr[f"{symbol1}_{symbol2}"] = (
                            returns_data[symbol1]
                            .rolling(30)
                            .corr(returns_data[symbol2])
                        )
            ...
            for symbol2 in symbols[i + 1 :]:
                if symbol1 in returns_data.columns and symbol2 in returns_data.columns:
                    try:
                        ...
                        print(
                            f"Warning: Regression failed for {symbol1} vs {symbol2}: {e}"
                        )
                        continue
            ...
            for symbol2 in symbols[i + 1 :]:
                if symbol1 in price_data.columns and symbol2 in price_data.columns:
                    try:
                        ...
                        relationship = (
                            "Cointegrated" if is_cointegrated else "Not Cointegrated"
                        )
                        ...
                        print(
                            f"Warning: Cointegration test failed for {symbol1}-{symbol2}: {e}"
                        )
                        continue
    ...
    def plot_correlation_heatmap(
        self, correlation_matrix: pd.DataFrame, title: str, symbols: List[str]
    ) -> str:
    ...
    def plot_regression_analysis(
        self, regression_results: Dict, symbols: List[str]
    ) -> str:
    ...
    def plot_rolling_correlations(
        self, rolling_correlations: Dict, symbols: List[str]
    ) -> str:
        ...
        plt.axhline(
            y=0.5, color="red", linestyle="--", alpha=0.5, label="High Correlation"
        )
        plt.axhline(
            y=-0.5,
            color="red",
        ...
        plt.title(
            "Rolling 30-Day Correlations Between Stocks", fontsize=14, fontweight="bold"
        )
        plt.xlabel("Date")
    ...
    def comprehensive_analysis(self, symbols: List[str], period: str = "2y") -> Dict:
        ...
        price_heatmap = self.plot_correlation_heatmap(
            regression_results["price_correlation"], "Price Correlation Matrix", symbols
        )
        ...
        regression_plot = self.plot_regression_analysis(
            regression_results["regression_results"], symbols
        )

        # Rolling correlation plots
        rolling_plot = self.plot_rolling_correlations(
            regression_results["rolling_correlations"], symbols
        )
    ...
    def print_analysis_summary(self, results: Dict, symbols: List[str]):
        ...
        if "regression_results" in results and results["regression_results"]:
            print(f"\n REGRESSION ANALYSIS:")
            print(
                f"{'Pair':<15} {'Beta':<8} {'R²':<8} {'Correlation':<12} {'Significance':<15}"
            )
            print(f"{'─' * 70}")
            ...
            for pair_name, reg_results in results["regression_results"].items():
                ...
                print(
                    f"{pair_display:<15} {reg_results['beta']:<8.3f} "
        ...
        if "cointegration_results" in results and results["cointegration_results"]:
            ...
            print(
                f"{'Pair':<15} {'Test Stat':<12} {'P-Value':<10} {'Relationship':<15}"
            )
            print(f"{'─' * 60}")
            ...
            for pair_name, coint_results in results["cointegration_results"].items():
                ...
                pair_display = f"{symbol1}-{symbol2}"
                print(
                    f"{pair_display:<15} {coint_results['test_statistic']:<12.3f} "
